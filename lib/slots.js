/** // BookEazy slots v2 — wall-clock time, business-local hours
 * slots.js — mirrors Flutter's SupabaseService.slotOptionsForDay exactly.
 * All slot times are treated as wall-clock (timezone-naive) so 11am at the
 * business always shows as 11am on the booking form regardless of viewer location.
 */
...
// Wall-clock key: YYYY-MM-DDTHH:mm — no UTC conversion.
// 11:00 AM is always "11:00" regardless of timezone.
function slotKey(dateLike) {
  const dt = new Date(dateLike);
  const pad = n => String(n).padStart(2, '0');
  // Use UTC components to match how Flutter stores times:
  // Flutter: dateTime.toUtc().toIso8601String() → "2026-03-29T05:30:00Z" for 11am IST
  // JS toISOString() → same "2026-03-29T05:30:00.000Z"
  // So both manual and online bookings produce the same UTC-based key
...
function rowBlocksSlot(row, slot, slotDurationMins) {
 ...
 const pad = n => String(n).padStart(2, '0');
  const slotDate = `${slot.getFullYear()}-${pad(slot.getMonth()+1)}-${pad(slot.getDate())}`;
...
export function generateSlots(schedule, blockedHours, appointments, date) {
  ...
  // Use local (wall-clock) time for slot generation
 const endTime = new Date(date);
  endTime.setHours(endH, endM, 0, 0);

  const now = new Date();

  // Build slot counts using wall-clock keys so 11am always matches 11am
  const slotCounts = {};
  ...
  console.log('[slots] appointments raw:', (appointments || []).map(a => ({ slot_start: a.slot_start, date_time: a.date_time })));
  console.log('[slots] slotCounts:', JSON.stringify(slotCounts));
  console.log('[slots] sample cursor key:', slotKey(new Date(date.getFullYear(), date.getMonth(), date.getDate(), 11, 0, 0)));
  console.log('[slots] maxPerSlot:', maxPerSlot);
  ...
  const slots = [];
  // Use local (wall-clock) time for cursor
