 ...
 *     open_mon…open_sun  BOOL   individual columns (Flutter saves these)
 *     open_days          JSONB  fallback { "1": true, ... } (legacy)
 *
 *   blocked_hours:
 *     is_recurring       BOOL
...
export function generateSlots(schedule, blockedHours, appointments, date) {
  ...
  // Flutter saves open days as individual boolean columns: open_mon…open_sun
  // Map to Flutter weekday convention (Mon=1 … Sun=7)
  const openDaysCols = {
    1: schedule.open_mon,
    2: schedule.open_tue,
    3: schedule.open_wed,
  ...
  // Fallback: legacy open_days JSONB column
  const openDaysJson = schedule.open_days || {};

  const hasColData = Object.values(openDaysCols)
    .some(v => v !== null && v !== undefined);

  if (hasColData) {
    // Use individual columns — primary source
  ...
  // If neither source has data → treat all days as open (safe fallback)
  ...
  // Count bookings per slot — normalise UTC ISO keys
  const slotCounts = {};
  for (const appt of (appointments || [])) {
    const key = appt.slot_start || appt.date_time;
    if (key) {
      const normKey = key.endsWith('Z') ? key : key + (key.includes('+') ? '' : 'Z');
      // Also store without milliseconds for flexible matching
      const noMs = normKey.replace(/\.\d{3}Z$/, 'Z');
      slotCounts[normKey] = (slotCounts[normKey] || 0) + 1;
      if (noMs !== normKey) slotCounts[noMs] = (slotCounts[noMs] || 0) + 1;
    }
  }
  ...
  while (cursor < endTime) {
    ...
    // Match against both full ISO and no-milliseconds variant
    const slotUtcKey     = slotCopy.toISOString();
    const slotUtcKeyNoMs = slotUtcKey.replace(/\.\d{3}Z$/, 'Z');
    const booked = slotCounts[slotUtcKey] || slotCounts[slotUtcKeyNoMs] || 0;
